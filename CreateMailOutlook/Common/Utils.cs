...
public static class Utils
{
    ...
    {
        Random random = new Random();
        const string charsUp = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        const string number = "0123456789";
        const string chartDown = "qwertyuiopasdfghjklzxcvbnm";
        string pass = new string(Enumerable.Repeat(charsUp, 3).Select(s => s[random.Next(s.Length)]).ToArray()).ToUpper() +
            new string(Enumerable.Repeat(number, 2).Select(s => s[random.Next(s.Length)]).ToArray()).ToUpper() +
            new string(Enumerable.Repeat(chartDown, 3).Select(s => s[random.Next(s.Length)]).ToArray()).ToUpper();
        return pass;
    }
